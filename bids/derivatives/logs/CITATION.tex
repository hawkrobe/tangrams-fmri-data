...
imaging (EPI) references with \texttt{topup} (\citet{topup}; FSL None).
...
and the T1w template. The following template was were selected for
...
the transform. BOLD runs were slice-time corrected to 0.708s (0.5 of
slice acquisition range 0s-1.42s) using \texttt{3dTshift} from AFNI
\citep[RRID:SCR\_005927]{afni}. The BOLD reference was then
...
nomenclature): \emph{fsaverage6}. All resamplings can be performed with
...
rigid-registration to the target EPI (echo-planar imaging) reference
run. The field coefficients were mapped on to the reference EPI using
the transform. BOLD runs were slice-time corrected to 0.709s (0.5 of
slice acquisition range 0s-1.42s) using \texttt{3dTshift} from AFNI
\citep[RRID:SCR\_005927]{afni}. The BOLD reference was then
co-registered to the T1w reference using \texttt{bbregister}
(FreeSurfer) which implements boundary-based registration \citep{bbr}.
...
\subsubsection{References}\label{references}}
  ...
  \bibliography{/project/data/bids/derivatives/logs/CITATION.bib}
